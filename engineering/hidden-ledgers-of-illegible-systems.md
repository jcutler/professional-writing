...
Resilience Engineering would say that some of the work at 2AM is genuine sense-making, novel reasoning about a failure nobody foresaw that no error message could have pre-empted. That work is not a tax, it's the job, the most valuable thing your engineers do. But that's exactly the point: sense-making capacity is finite, and worth protecting. Every hour of it consumed recovering facts the system already had — which service failed, what state the request was in, what the blast radius is — is an hour billed against a hidden ledger. The diagnosis tax is not levied on the unknowable. It is levied on the knowable-but-withheld.
